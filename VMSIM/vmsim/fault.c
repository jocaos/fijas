...
static void fault_random(pte_t *pte, ref_kind_t type); //ok
static void fault_lfu(pte_t *pte, ref_kind_t type); // ok
static void fault_mfu(pte_t *pte, ref_kind_t type); // ok
static void fault_lru(pte_t *pte, ref_kind_t type); //ok
static void fault_fifo(pte_t *pte, ref_kind_t type); //ok
static void fault_clock(pte_t *pte, ref_kind_t type); //
static void fault_second(pte_t *pte, ref_kind_t type); //ok
...
// no recently page replace algorithm
void faul_nru (pte_t *pte, ref_kind_t type) {
	int i;
	int check = 1;
	int frame = 0;
	...
	if(check <= opts.phys_pages) {
		...
		for(i = 0; i < opts.phys_pages; i++) { // recorro la memoria física
...
void fault_lfu(pte_t *pte, ref_kind_t type) {
	...
	if(check <= opts.phys_pages) {
		...
		//to maintain FIFO order sacando el mas antiguo
		for(i = 0; i < opts.phys_pages; i++) {
...
void fault_fifo(pte_t *pte, ref_kind_t type) {
	...
	static int frame = 0;
	static int check = 1; // numero de pagina que estoy metiendo empieza en 1
	...
	if(check <= opts.phys_pages) {
		...
		min = physmem[0]->c;
		for(i = 0; i < opts.phys_pages; i++) { // se usa para llenar luego de usar toda la tabla
			if q(physmem[i]->c < min) {
				min = physmem[i]->c;
				loc = i; 
			}
		}
...
static void fault_clock(pte_t *pte, ref_kind_t type) {
	...
	if (check <= opts.phys_pages) {
		...
		clock_hand++; // trackea el siguiente lugar a buscar
...
static void fault_second(pte_t *pte, ref_kind_t type) {
	...
	//Finding the minimum
		...
		// ordenamiento de menor a mayor antiguedad
		for (i = 0; i < opts.phys_pages; i++) { 
